    ...
    @PathVariable("idoferta") String idoferta) {
        ...
        if (accept != null && accept.contains("application/json")) {
        	...
        	try {	
					...
					RegistrarRequest persona = ofertaRepository.findOne(idoferta);
					if(persona == null) {
						JsonApiBodyResponseErrors responseErrors1 = new JsonApiBodyResponseErrors();
						responseErrors1.setCode("58");
						responseErrors1.setDetalle("NO EXISTEN OFERTAS CON ESE ID");
						return new ResponseEntity<JsonApiBodyResponseErrors>(responseErrors1, HttpStatus.FAILED_DEPENDENCY);
					}
					else {
						JsonApiBodyRequest body_response = new JsonApiBodyRequest();
						List<RegistrarRequest> lista = new ArrayList<RegistrarRequest>();
						lista.add(persona);
						body_response.setOferta(lista);
						return new ResponseEntity<JsonApiBodyRequest>(body_response, HttpStatus.OK);       				
					}
//					
