        ...
        if (accept != null && accept.contains("application/json")) {
	    	...
	    	if (!body.getPersona().get(0).getContrasena().equals("")) {
		    	...
		    	try {
		    		...
		    		System.out.println("estamos en el try"+ correo.get(0).getciudad());
		    		if(!correo.isEmpty()) {
						...
						responseErrors.setCodigo("65");
		        		responseErrors.setDetalle("NO PODEMOS REGISTRARTE EN UNA ACTUALIZACION");
		        		return new ResponseEntity<JsonApiBodyResponseErrors>(responseErrors, HttpStatus.FAILED_DEPENDENCY);
					}
